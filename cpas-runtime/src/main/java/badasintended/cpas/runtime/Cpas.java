            ...
            server.execute(() -> {
                ...
                if (slotId.equals(REGULAR_SLOT)) {
                    if (cursor.isEmpty() || type.equipmentSlot == null || type.equipmentSlot == MobEntity.getPreferredEquipmentSlot(cursor)) {
                        player.currentScreenHandler.setCursorStack(player.getInventory().getStack(type.inventoryId).copy());
                        player.getInventory().setStack(type.inventoryId, cursor.copy());
                    ...
                    if (slot != null && (cursor.isEmpty() || slot.canEquip(cursor))) {
                        ...
                        player.currentScreenHandler.setCursorStack(slot.setStack(cursor.copy()).copy());
                        player.playerScreenHandler.syncState();
                    }
                }
            });
